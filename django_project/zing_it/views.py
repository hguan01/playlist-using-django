...
def about(request):
    try:
        thankyou_next = Song.objects.create(track="thank u, next", artist="Ariana Grande", album="thank u, next",
                                            length="3:27", playlist_id=1)
        one_kiss_next = Song.objects.create(track="One Kiss, next", artist="Dua Lipa, Calvin Harris", album="One Kiss",
                                            length="3:34", playlist_id=1)
        better_now = Song.objects.create(track="Better Now", artist="Post malone", album="beerbongs & bentleys",
                                         length="3:51", playlist_id=1)
        the_middle = Song.objects.create(track="The Middle", artist="Grey,Marren Morris, ZEDD", album="The Middle",
                                         length="3:04", playlist_id=1)
        ...
        if (password != confirm_password):
            ...
            try:
                ...
                return render(request, 'zing_it/signup.html', {"form": form,
                                                               "status": "This email already exists in the system! Please log in instead."})
            except Exception as e:
                print(e)
